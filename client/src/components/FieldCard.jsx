...
export default function FieldCard({ component, onSubmit, onUpload }) {
  ...
  const inputStyle = {
    ...
    background: "#fff", color: "#111827", fontSize: 14,
    fontFamily: "'Inter',sans-serif", outline: "none", width: "100%", boxSizing: "border-box",
  };
  const labelStyle = {
    fontSize: 11, color: "#C5A572", fontFamily: "'Inter',sans-serif",
                ...
                {["yes", "no"].map(opt => (
                  <button key={opt} onClick={() => handleChange(f.key, opt)}
                    style={{
                      ...
                      border: `1px solid ${values[f.key] === opt ? "#C5A572" : "#D1D5DB"}`,
                      background: values[f.key] === opt ? "rgba(197,165,114,0.13)" : "#fff",
                      color: values[f.key] === opt ? "#C5A572" : "#6B7280",
      ...
      {upload_label && onUpload && (
        <div style={{ marginBottom: 12 }}>
          ...
          <button onClick={() => fileRef.current?.click()}
            ...
            📄 {upload_label}
            {upload_hint && <span style={{ display: "block", fontSize: 10, color: "#9CA3AF", marginTop: 2 }}>{upload_hint}</span>}
          </button>
        </div>
      )}
